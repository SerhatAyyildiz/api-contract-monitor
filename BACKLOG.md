...
| `slow_response` yanlış alarm düzeltmesi | **G12'de yapıldı.** G8'deki oran-tabanlı eşik (3 katı) gerçek kullanımda üç yanlış alarm üretti — hızlı bir servisin ortalaması düşükken (17-105 ms) sıradan bir dalgalanma bile "yavaş" sayıldı. Mutlak bir alt sınır eklendi (`ASGARI_YAVAS_SURE_MS = 1000`): üç API'nin 119 gerçek çağrısı incelendi, hiçbiri hiçbir zaman 1 saniyeyi aşmadı (en yavaşı 546 ms). Sınır bu veriye dayanarak seçildi. Üç gerçek yanlış alarm yeniden oynatılıp artık tetiklenmediği, gerçek bir yavaşlama simülasyonunun hâlâ yakalandığı doğrulandı. |
